...
| `service-build-push.yml` | Build one service image tagged with the 7-char git SHA, **Trivy gate**, push to ECR via OIDC. No git write-back. | `service-name`, `service-path`, `ecr-registry`, `ecr-repository`, `role-to-assume` | — (OIDC) | `image-tag` |
